...
Экспериментально было получено, что погрешность одного измерения не превышает  `2%`, а из-за усреднения уменьшается в 10 раз(потому что тесты проводились 10 раз). 
...
В качестве итога можно отметить, что реализация на массивах является наиболее оптимальной в случает написания одного переносимого алгоритма. Это позволит компилятору самостоятельно провести оптимизации для нужной архитектуры. В случае написания алгоритма для какой-то конкретной машины предпочтительнее использовать реализацию на SIMD инструкциях.